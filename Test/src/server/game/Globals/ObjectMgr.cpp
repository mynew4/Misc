...
bool ObjectMgr::LoadTrinityStrings(const char* table, int32 min_value, int32 max_value)
{
    ...
    if (!result)
    {
        if (min_value == MIN_holysemu_string_ID)              // error only in case internal strings
    ...
    if (min_value == MIN_holysemu_string_ID)
...
const char *ObjectMgr::GetTrinityString(int32 entry, LocaleConstant locale_idx) const
{
    ...
    if (entry > 0)
        TC_LOG_ERROR(LOG_FILTER_SQL, "Entry %i not found in `holysemu_string` table.", entry);
    else
