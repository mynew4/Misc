...
void BattlegroundBFG::Update (uint32 diff)
{
...
void BattlegroundBFG::RemovePlayer (Player * /*plr*/, uint64 /*guid*/, uint32 /*value*/) { }
...
void BattlegroundBFG::HandleAreaTrigger (Player *Source, uint32 Trigger)
{
    ...
    case 4021:          // Unk2
        //break;
    default:
        sLog->outError(LOG_FILTER_MAPS, "WARNING: Unhandled AreaTrigger in Battleground: %u", Trigger);
...
void BattlegroundBFG::EventPlayerClickedOnFlag (Player *source, GameObject* /*target_obj*/)
{
    ...
    if (node == BG_BG_DYNAMIC_NODES_COUNT)
    {
        sLog->outInfo(LOG_FILTER_PLAYER, "Player %s (GUID: %u) in Battle for Gilneas fired EventPlayerClickedOnFlag() but isn't near any flag", source->GetName(), source->GetGUIDLow());
...
bool BattlegroundBFG::SetupBattleground ()
{
    for (int i = 0; i < BG_BG_DYNAMIC_NODES_COUNT; ++i)
    {
        ...
        {
            sLog->outError(LOG_FILTER_BATTLEGROUND, "BattlegroundBFG: Failed to spawn some objects, Battleground not created!");
            return false;
        }
    }
    ...
    {
        sLog->outError(LOG_FILTER_BATTLEGROUND, "BattlegroundBFG: Failed to spawn door object, Battleground not created!");
        return false;
    }
    //buffs
    for (int i = 0; i < BG_BG_DYNAMIC_NODES_COUNT; ++i)
    {
        if (!AddObject(BG_BG_OBJECT_SPEEDBUFF_LIGHTHOUSE + 3 * i, Buff_Entries[0], BG_BG_BuffPositions[i][0], BG_BG_BuffPositions[i][1], BG_BG_BuffPositions[i][2], BG_BG_BuffPositions[i][3], 0, 0, sin(BG_BG_BuffPositions[i][3] / 2), cos(BG_BG_BuffPositions[i][3] / 2), RESPAWN_ONE_DAY) || !AddObject(BG_BG_OBJECT_SPEEDBUFF_LIGHTHOUSE + 3 * i + 1, Buff_Entries[1], BG_BG_BuffPositions[i][0], BG_BG_BuffPositions[i][1], BG_BG_BuffPositions[i][2], BG_BG_BuffPositions[i][3], 0, 0, sin(BG_BG_BuffPositions[i][3] / 2), cos(BG_BG_BuffPositions[i][3] / 2), RESPAWN_ONE_DAY) || !AddObject(BG_BG_OBJECT_SPEEDBUFF_LIGHTHOUSE + 3 * i + 2, Buff_Entries[2], BG_BG_BuffPositions[i][0], BG_BG_BuffPositions[i][1], BG_BG_BuffPositions[i][2], BG_BG_BuffPositions[i][3], 0, 0, sin(BG_BG_BuffPositions[i][3] / 2), cos(BG_BG_BuffPositions[i][3] / 2), RESPAWN_ONE_DAY))
            sLog->outError(LOG_FILTER_BATTLEGROUND, "BattlegroundBFG: Failed to spawn buff object!");
    }
