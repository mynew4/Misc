...
struct BfTBGameObjectBuilding
{
    ...
    void Damaged()
    {
        ...
        if(m_NameId)
            m_TB->SendWarningToAllInZone(BATTLEFIELD_TB_TEXT_TOWER_DAMAGE);
    }
    ...
    void Destroyed()
    {
        ...
        //Warning
        if(m_NameId)
            m_TB->SendWarningToAllInZone(BATTLEFIELD_TB_TEXT_TOWER_DESTROY);
        switch(m_Type)
        {
    ...
    void ChangeControl(uint8 team,bool init/* for first call in setup*/)
    {
        switch (team)
        {
            case BATTLEFIELD_TB_TEAM_NEUTRAL:
            {
                ...
                m_TB->SendWarningToAllInZone(BATTLEFIELD_TB_TEXT_FORT_ATTACK);
            ...
            case BATTLEFIELD_TB_TEAM_ALLIANCE:
            {
                ...
                //Warning message
                if(!init)
                    m_TB->SendWarningToAllInZone(BATTLEFIELD_TB_TEXT_FORT_TAKEN);
