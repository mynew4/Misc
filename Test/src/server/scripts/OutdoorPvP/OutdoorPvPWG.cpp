...
bool OutdoorPvPWG::SetupOutdoorPvP()
{
    ...
    if (!result)
        TC_LOG_ERROR(LOG_FILTER_OUTDOORPVP, "Cannot find siege workshop master or spirit guides in creature!");
    else
    {
        do
        {
    ...
    do
    {
        ...
        if (capturePointEntry)
        {
            ...
            if (!engGuid)
            {
                TC_LOG_ERROR(LOG_FILTER_OUTDOORPVP, "Cannot find nearby siege workshop master!");
                continue;
            }
            else
            ...
            {
                delete workshop;
                TC_LOG_ERROR(LOG_FILTER_OUTDOORPVP, "Cannot add capture point!");
                continue;
            }
    ...
    if (!m_gate)
    {
        TC_LOG_ERROR(LOG_FILTER_OUTDOORPVP, "Cannot find wintergrasp fortress gate!");
        return false;
    }
    ...
    else if (obj->GetGoType() == GAMEOBJECT_TYPE_DESTRUCTIBLE_BUILDING)
    {
        ...
        if (eventId == obj->GetGOInfo()->building.damagedEvent)
        {
            ...
            switch(state->type)
            {
                ...
                {
                    msgStr = ffmtstring(sObjectMgr->GetTrinityStringForDBCLocale(LANG_BG_WG_WORKSHOP_DAMAGED), msgStr.c_str(), sObjectMgr->GetTrinityStringForDBCLocale(getDefenderTeam() == TEAM_ALLIANCE ? LANG_BG_AB_ALLY : LANG_BG_AB_HORDE));
                ...
                case BUILDING_WALL:
                {
                    ...
                    for (PlayerSet::iterator itr = m_players[getDefenderTeam()].begin(); itr != m_players[getDefenderTeam()].end(); ++itr)
                    {
						if (Player* plr= ObjectAccessor::FindPlayer(*itr))
						{
							...
							plr->PlayDirectSound(TeamIDsound) ; // Wintergrasp Fortress under Siege
						}
					}
                ...
                case BUILDING_TOWER:
                {
                    ...
                    msgStr = ffmtstring(sObjectMgr->GetTrinityStringForDBCLocale(LANG_BG_WG_TOWER_DAMAGED), msgStr.c_str());
                    sWorld->SendZoneText(ZONE_WINTERGRASP, msgStr.c_str());
                    for (PlayerSet::iterator itr = m_players[getDefenderTeam()].begin(); itr != m_players[getDefenderTeam()].end(); ++itr)
                    {
						if (Player* plr= ObjectAccessor::FindPlayer(*itr))
        ...
        else if (eventId == obj->GetGOInfo()->building.destroyedEvent)
        {
            ...
            switch(state->type)
            {
                case BUILDING_WORKSHOP:
                {
                    ...
                    msgStr = ffmtstring(sObjectMgr->GetTrinityStringForDBCLocale(LANG_BG_WG_WORKSHOP_DESTROYED), msgStr.c_str(), sObjectMgr->GetTrinityStringForDBCLocale(getDefenderTeam() == TEAM_ALLIANCE ? LANG_BG_AB_ALLY : LANG_BG_AB_HORDE));
                    sWorld->SendZoneText(ZONE_WINTERGRASP, msgStr.c_str());
                }
                    break;
                case BUILDING_WALL:
                ...
                case BUILDING_TOWER:
                {
                    ...
                    msgStr = ffmtstring(sObjectMgr->GetTrinityStringForDBCLocale(LANG_BG_WG_TOWER_DESTROYED), msgStr.c_str());
                    sWorld->SendZoneText(ZONE_WINTERGRASP, msgStr.c_str());
                    for (PlayerSet::iterator itr = m_players[getDefenderTeam()].begin(); itr != m_players[getDefenderTeam()].end(); ++itr)
                    {
						if (Player* plr= ObjectAccessor::FindPlayer(*itr))
						...
						// Add Support of Quests Toppling the Towers & Southern Sabotage
							...
							plr->RewardPlayerAndGroupAtEvent(TOWER_PVP_DESTROYED, obj);
						}
					}
                    for (PlayerSet::iterator itr = m_players[getAttackerTeam()].begin(); itr != m_players[getAttackerTeam()].end(); ++itr)
                    {
						if (Player* plr= ObjectAccessor::FindPlayer(*itr))
						{
							if (getDefenderTeam()==TEAM_ALLIANCE)
								...
								TeamIDsound=OutdoorPvP_WG_ALLIANCE_CAPTAIN;  //Horde Worn Sound
							plr->PlayDirectSound(TeamIDsound) ; // Wintergrasp Fortress destroyed Siege
						}
					}
                }
                    break;
...
void OutdoorPvPWG::ModifyWorkshopCount(TeamId team, bool add)
{
    ...
    if (add)
        ...
        TC_LOG_ERROR(LOG_FILTER_OUTDOORPVP, "OutdoorPvPWG::ModifyWorkshopCount: negative workshop count!");
...
void OutdoorPvPWG::BroadcastStateChange(BuildingState *state) const
{
    if (m_sendUpdate)
        for (uint32 team = 0; team < 2; ++team)
            for (PlayerSet::const_iterator plr = m_players[team].begin(); plr != m_players[team].end(); ++plr)
...
{
    if (!sWorld->getBoolConfig(CONFIG_WINTERGRASP_ENABLE) || !isWarTime())
        return;
    ...
    if (m_timer > diff)
    {
        ...
        if (isWarTime())
        {
            if (m_timer != 1) // 1 = forceStopBattle
                sWorld->SendZoneText(ZONE_WINTERGRASP, ffmtstring(sObjectMgr->GetTrinityStringForDBCLocale(entry), sObjectMgr->GetTrinityStringForDBCLocale(getDefenderTeam() == TEAM_ALLIANCE ? LANG_BG_AB_ALLY : LANG_BG_AB_HORDE)));
            EndBattle();
        } else {
            if (m_timer != 1)
...
void OutdoorPvPWG::forceChangeTeam()
{
    ...
    sWorld->SendZoneText(ZONE_WINTERGRASP, ffmtstring(sObjectMgr->GetTrinityStringForDBCLocale(LANG_BG_WG_SWITCH_FACTION), sObjectMgr->GetTrinityStringForDBCLocale(getAttackerTeam() == TEAM_ALLIANCE ? LANG_BG_AB_ALLY : LANG_BG_AB_HORDE)));
    if (isWarTime())
        forceStartBattle();
    else
        forceStopBattle();
...
void OutdoorPvPWG::EndBattle()
{
    ...
    for (PlayerSet::iterator itr = m_players[getDefenderTeam()].begin(); itr != m_players[getDefenderTeam()].end(); ++itr)
    {
		if (Player* plr= ObjectAccessor::FindPlayer(*itr))
		{
			...
			plr->PlayDirectSound(TeamIDsound) ; // SoundOnEndWin
		}
	}
    for (PlayerSet::iterator itr = m_players[getAttackerTeam()].begin(); itr != m_players[getAttackerTeam()].end(); ++itr)
		if (Player* plr= ObjectAccessor::FindPlayer(*itr))
		{        
			plr->PlayDirectSound(OutdoorPvP_WG_SOUND_NEAR_VICTORY) ; // SoundOnEndLoose
		}
	for (uint32 team = 0; team < 2; ++team)
    {
...
void OutdoorPvPWG::RewardMarkOfHonor(Player *plr, uint32 count)
{
    ...
    {
        TC_LOG_ERROR(LOG_FILTER_OUTDOORPVP,"Wintergrasp reward item (Entry %u) not exist in `item_template`.", WG_MARK_OF_HONOR);
        return;
    }
