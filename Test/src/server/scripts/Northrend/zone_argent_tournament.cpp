...
/*######## Quest Soporte Threat From Above## npc_Chillmaw######*/
    ...
    struct npc_argent_valiantAI : public ScriptedAI
    {
        ...
        void DamageTaken(Unit* pDoneBy, uint32& uiDamage)
        {
            if(pDoneBy)
            {
                ...
                {
                    Talk(NPC_ARGENT_VAILIANT_TEXT_SAY_WIN);
                    uiDamage = 0;
        ...
        void KilledUnit(Unit* /*victim*/)
        {
            me->setFaction(35);
            me->DespawnOrUnsummon(5000);
            Talk(NPC_ARGENT_VAILIANT_TEXT_SAY_LOOSE);
            me->CombatStop(true);
        }
        ...
        void EnterCombat(Unit* /*who*/)
        {
            Talk(NPC_ARGENT_VAILIANT_TEXT_SAY_START);
        }
...
class npc_valiant_grand_melee : public CreatureScript
{
public:
    npc_valiant_grand_melee() : CreatureScript("npc_valiant_grand_melee") { }
    
    struct npc_valiant_grand_meleeAI : public ScriptedAI
    {
        Unit *pTarget;
        uint32 SpellTimer,MoviTimer;
        int8 x,y;    
       
        npc_valiant_grand_meleeAI(Creature* pCreature) : ScriptedAI(pCreature){}
    ...
    CreatureAI *GetAI(Creature *creature) const
    {
        return new npc_valiant_grand_meleeAI(creature);
    }
};
...
#define SAY_START_VALIANT       "You believe you are ready to be a champion? Defend yourself!"  
#define SAY_END_VALIANT         "Most impressive. You are worthy to gain the rank of champion"
...
public:
    ...
    struct npc_argent_championAI : public ScriptedAI
    {
        ...
        void DamageTaken(Unit* pDoneBy, uint32& uiDamage)
        {
            if(pDoneBy)
            {
                ...
                { 
                    ...
                    me->MonsterSay(SAY_END_VALIANT, LANG_UNIVERSAL, 0);
                    uiDamage = 0;
        ...
        void EnterCombat(Unit* /*who*/)
        {
            me->MonsterSay(SAY_START_VALIANT, LANG_UNIVERSAL, 0);
            SpellTimer= urand(TIMER_SPELL_MIN,TIMER_SPELL_MAX);
            MoviTimer = urand(TIMER_MoviTimer_MIN,TIMER_MoviTimer_MAX);
        }
...
/*######## Quest 13042 ## "Deep in the Bowels of The Underhalls"######*/
enum eOsterkilgr
{
    SAY_QUEST1                                   = -1603500,
    SAY_QUEST2                                   = -1603501,    
...
/*######## Quest 13231 - 13228 (A/H) - 13232## "The Broken Front"## "Finish Me!"######*/

/*######## npc_dying_soldier######*/

#define GOSSIP_ITEM_END "Travel well, hero of the Alliance."

enum
{
	SPELL_DESPAWN_DYING_SOLDIER = 58959,
	NPC_DYING_SOLDIER = 31312,

	QUEST_AVENGE_ME = 13230, // Horde side
	QUEST_FINISH_ME = 13232, // Alliance side
};

bool GossipHello_npc_dying_soldier(Player* pPlayer, Creature* pCreature)
{
    	if (pCreature->isQuestGiver())
	{
        	pPlayer->PrepareQuestMenu(pCreature->GetGUID());
	...
	if (pPlayer->GetQuestStatus(QUEST_AVENGE_ME) == QUEST_STATUS_INCOMPLETE || pPlayer->GetQuestStatus(QUEST_FINISH_ME) == QUEST_STATUS_INCOMPLETE)
	{
		pPlayer->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, GOSSIP_ITEM_END, GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF + 1);
	}
...
bool GossipSelect_npc_dying_soldier(Player* pPlayer, Creature* pCreature, uint32 uiSender, uint32 uiAction)
{
	if (uiAction == GOSSIP_ACTION_INFO_DEF + 1)
	{
		pPlayer->CLOSE_GOSSIP_MENU();
		pPlayer->KilledMonsterCredit(NPC_DYING_SOLDIER, 0);
...
public:
    ...
    bool OnGossipSelect(Player* pPlayer, Creature* pCreature, uint32 uiSender, uint32 uiAction)
    {
        ...
        if (uiAction == GOSSIP_SENDER_INFO)
        {
            ...
            Talk(RAND(SAY_KILL_QUEST1, SAY_KILL_QUEST2, SAY_KILL_QUEST3, SAY_KILL_QUEST4, SAY_KILL_QUEST5), pCreature);
        }

        if (uiAction == GOSSIP_SENDER_INFO+1)
        {
            pPlayer->CLOSE_GOSSIP_MENU();
            Talk(SAY_QUEST_SOLDIER1, pCreature);
        ...
        void UpdateAI(const uint32 uiDiff)
        {
            if (bTalkTime && uiTalkTimer <= uiDiff)
            {
                if (Player *pPlayer = Unit::GetPlayer(*me, uiPlayerGUID))
                {
                    ...
                    {
                        case 0:
                            Talk(SAY_QUEST_SOLDIER2);
                            uiTalkTimer = 3000;
                            ++uiStep;
                            break;
                        case 1:
                            Talk(SAY_QUEST_SOLDIER3);
                            uiTalkTimer = 3000;
                            ++uiStep;
                            break;
                        case 2:
                            Talk(SAY_QUEST_SOLDIER4);
                            uiTalkTimer = 3000;
                            ++uiStep;
                            break;
                        case 3:
                            Talk(SAY_QUEST_SOLDIER5);
                            uiTalkTimer = 3000;
                            ++uiStep;
                            break;
                        case 4:
                            Talk(SAY_QUEST_SOLDIER6);
                            uiTalkTimer = 3000;
                            ++uiStep;
                            break;
                        case 5:
                            Talk(SAY_QUEST_SOLDIER7);
                            if (pPlayer->GetQuestStatus(QUEST_THE_BROKEN_FRONT_A) == QUEST_STATUS_INCOMPLETE)
                                pPlayer->KilledMonsterCredit(CREATURE_DYING_SOLDIER_KC, 0);
                            bTalkTime = false;
                            uiStep = 0;
...
public:
    ...
    bool OnGossipSelect(Player* pPlayer, Creature* pCreature, uint32 uiSender, uint32 uiAction)
    {
        ...
        {
            pPlayer->CLOSE_GOSSIP_MENU();
            Talk(SAY_QUEST_BERSERKER1, pCreature);
        ...
        void UpdateAI(const uint32 uiDiff)
        {
            if (bTalkTime && uiTalkTimer <= uiDiff)
            {
                if (Player *pPlayer = Unit::GetPlayer(*me, uiPlayerGUID))
                {
                    ...
                    {
                        case 0:
                            Talk(SAY_QUEST_BERSERKER2);
                            uiTalkTimer = 3000;
                            ++uiStep;
                            break;
                        case 1:
                            Talk(SAY_QUEST_BERSERKER3);
                            uiTalkTimer = 3000;
                            ++uiStep;
                            break;
                        case 2:
                            Talk(SAY_QUEST_BERSERKER4);
                            uiTalkTimer = 3000;
                            ++uiStep;
                            break;
                        case 3:
                            Talk(SAY_QUEST_BERSERKER5);
                            uiTalkTimer = 3000;
                            ++uiStep;
                            break;
                        case 4:
                            Talk(SAY_QUEST_BERSERKER6);
                            uiTalkTimer = 3000;
                            ++uiStep;
                            break;
                        case 5:
                            Talk(SAY_QUEST_BERSERKER7);
                            uiTalkTimer = 3000;
                            ++uiStep;
                            break;
                        case 6:
                            Talk(SAY_QUEST_BERSERKER8);
                            if (pPlayer->GetQuestStatus(QUEST_THE_BROKEN_FRONT_H) == QUEST_STATUS_INCOMPLETE)
