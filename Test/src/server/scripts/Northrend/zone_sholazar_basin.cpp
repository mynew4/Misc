...
public:
    ...
    struct npc_generic_oracle_treasure_triggerAI: public ScriptedAI
    {
        ...
        Player* GetNearPlayer()
        {
            ...
            if (me->SelectNearestPlayer(radius) && (me->SelectNearestPlayer(radius)->GetQuestStatus(QUEST_APPEASING_THE_GREAT_RAIN_STONE) == QUEST_STATUS_INCOMPLETE || me->SelectNearestPlayer(radius)->GetQuestStatus(QUEST_GODS_LIKE_SHINEY_THINGS) == QUEST_STATUS_INCOMPLETE))
                return me->SelectNearestPlayer(radius);
            return NULL;
        }
