...
public:
    npc_archmage_vargoth() :
            CreatureScript("npc_archmage_vargoth") {
    }
    ...
    bool OnGossipHello(Player* player, Creature* creature) {
        ...
        if (player->HasItemCount(ITEM_ACANE_MAGIC_MASTERY, 1, false)) {
            if (!player->HasSpell(SPELL_FAMILAR_PET)
                    && !player->HasItemCount(ITEM_FAMILAR_PET, 1, true))
...
public:
    ...
    bool OnGossipHello(Player* player, Creature* creature) {
        ...
        if (player->HasAchieved(ACHIEVEMENT_HIGHER_LEARNING)
                && !player->HasItemCount(ITEM_THE_SCHOOLS_OF_ARCANE_MAGIC, 1,
                        true))
...
public:
    ...
    bool OnGossipHello(Battlefield* player, Creature* creature)
    {
