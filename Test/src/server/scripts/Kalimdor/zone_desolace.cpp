...
#define smeed_text_1 "Hey, look out with that Kodo! You had better inspect that beast before i give you credit!"

class npc_smeed : public CreatureScript
{
public:
    npc_smeed() : CreatureScript("npc_smeed") { }
    bool OnGossipHello(Player *pPlayer, Creature* pCreature)
    {
            uint32 minionssize;
            std::list<Creature*> minions1;
            std::list<Creature*> minions2;
            if (pPlayer->hasQuest(5561) && pPlayer->GetQuestStatus(5561) == QUEST_STATUS_INCOMPLETE)
            {
                pCreature->MonsterSay(smeed_text_1,LANG_UNIVERSAL,NULL);
                pPlayer->GetAllMinionsByEntry(minions1,4702);
                pPlayer->GetAllMinionsByEntry(minions2,4700);
                minionssize = minions1.size() + minions2.size();
                if(minionssize >= 3)
                {
                    pPlayer->CompleteQuest(5561);
                    pPlayer->RemoveAllMinionsByEntry(4702);
                    pPlayer->RemoveAllMinionsByEntry(4700);
