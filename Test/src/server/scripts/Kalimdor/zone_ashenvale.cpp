...
class npc_demoralized_peon : public CreatureScript
{
    public:
        npc_demoralized_peon() : CreatureScript("npc_demoralized_peon") {}        
        bool OnGossipHello(Player* pPlayer, Creature* pCreature)
        {
            if (pPlayer->GetQuestStatus(2744) == QUEST_STATUS_INCOMPLETE)
                CAST_AI(npc_demoralized_peon::npc_demoralized_peonAI, pCreature->AI())->Start = true;
            return true;
        }       
        struct npc_demoralized_peonAI : public ScriptedAI
        {
            npc_demoralized_peonAI(Creature *c) : ScriptedAI(c) {}
            bool Start;
            bool Summoned;
            void EnterCombat(Unit* /*who*/) {}
            void Reset()
            ...
            {
                if(summon->GetEntry() == 33458)
                {
                    me->SummonGameObject(194349,me->GetPositionX(), me->GetPositionY(), me->GetPositionZ(), me->GetOrientation(),0,0,0,0,9000000);
                    me->DisappearAndDie();
                }
            }
            void UpdateAI(const uint32 diff)
            {
                ...
                if(!Summoned)
                {
                    me->SummonCreature(33458,me->GetPositionX(),me->GetPositionY(),me->GetPositionZ(),me->GetOrientation(),TEMPSUMMON_DEAD_DESPAWN, 1000);
                    Summoned = true;
                }
            }
        };
        CreatureAI* GetAI(Creature* creature) const
        {
            return new npc_demoralized_peonAI(creature);
        }
};
