	...
	struct boss_shannoxAI : public BossAI
	{
		...
		void UpdateAI(const uint32 diff)
		{
			...
			if ((pRiplimb->isDead() || pRageface -> isDead()) && !softEnrage)
			{
				...
				Talk(SAY_ON_DOGS_FALL); //TODO Test
		...
		void UpdateAI(const uint32 diff)
		{
			...
			while (uint32 eventId = events.ExecuteEvent())
			{
				...
				case EVENT_TAKING_SPEAR_DELAY:
					...
					me->GetMotionMaster()->MovePoint(MOTION_SLOT_IDLE,GetSpear()->GetPositionX(),GetSpear()->GetPositionY(),GetSpear()->GetPositionZ());
					break;
				...
				if(!me->HasAura(SPELL_WARY_10N))
				{
					...
					if (GetSpear() != NULL)
					{
						if (instance->GetData(DATA_CURRENT_ENCOUNTER_PHASE) == PHASE_RIPLIMB_GOS_TO_SPEER && GetSpear()->GetDistance(me) <= 1)
						{
							...
							me->GetMotionMaster()->MoveIdle();
							me->GetMotionMaster()->MovePoint(MOTION_SLOT_IDLE,GetShannox()->GetPositionX(),GetShannox()->GetPositionY(),GetShannox()->GetPositionZ());
