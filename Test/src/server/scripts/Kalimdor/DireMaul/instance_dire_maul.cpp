...
{
public:
	instance_dire_maul() : InstanceMapScript("instance_dire_maul", 671) { }
	...
	struct instance_dire_maul_InstanceMapScript : public InstanceScript
	{
		...
		uint64 uiMizzle;
		// Gameobjects
		uint64 uiConservatoryDoor;
		uint64 uiCrumbleWall;
		uint64 uiCorruptVine;
		uint64 uiFelvineShard;
		uint64 uiNorthLibraryDoor;
		uint64 uiForcefield;
		uint64 uiWarpwoodDoor;
		uint64 uiWestLibraryDoor;
		uint64 uiPrincesChestAura;
		uint64 uiCrystalGenerator1;
		uint64 uiCrystalGenerator2;
		uint64 uiCrystalGenerator3;
		uint64 uiCrystalGenerator4;
		uint64 uiCrystalGenerator5;
		...
		void Initialize()
			...
			uiConservatoryDoor				= 0;
			uiCrumbleWall					= 0;
			uiCorruptVine					= 0;
			uiFelvineShard					= 0;
			uiNorthLibraryDoor				= 0;
			uiForcefield					= 0;
			uiWarpwoodDoor					= 0;
			uiWestLibraryDoor				= 0;
			uiPrincesChestAura				= 0;
			uiCrystalGenerator1				= 0;
			uiCrystalGenerator2				= 0;
			uiCrystalGenerator3				= 0;
			uiCrystalGenerator4				= 0;
			uiCrystalGenerator5				= 0;

			for (uint8 i = 0; i < MAX_ENCOUNTER; ++i)
				Encounter[i] = NOT_STARTED;
			int m_bWallDestroyed(false),
			int m_bDoNorthBeforeWest(false),
			memset();
		}
...
InstanceScript* GetInstanceScript_instance_dire_maul(Map* pMap)
{
