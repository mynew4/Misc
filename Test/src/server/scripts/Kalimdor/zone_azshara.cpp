...
public:
    ...
    struct mobs_spitelashesAI : public ScriptedAI
    {
        ...
        {            // we mustn't remove the Creature in the same round in which we cast the summon spell, otherwise there will be no summons
            ...
            if (!UpdateVictim())
                return;            /// @todo add abilities for the different creatures
        ...
        void UpdateAI(uint32 diff)
        {
            ...
            if (!Escape)
            {
                ...
                {                    // temp solution - unit can't be teleported by core using spelleffect 5, only players
                    DoTeleportTo(3706.39f, -3969.15f, 35.9118f);                    //begin swimming and summon depth charges
...
/*##### npc_bilgewater_foreman####*/

#define FOREMAN_GOSSIP_OPTION_1 "Nobody shorts Garrosh Hellscream. You finish those ships - and finish them right - or this gets personal."
#define SAY_START_1 "Nobody tells me how to run my business here"
#define BATTLE_SAY_1 "It was just a couple of rivets, That ship had plenty!"
#define BATTLE_SAY_2 "I'm sure it would've seaworthy in calm waters"
#define BATTLE_SAY_3 "Less rivets mean a cleaner enviroment!"
#define BATTLE_SAY_4 "...Can we settle this with cash?"
#define BATTLE_END "Okay okay, stop! Stop! You make a compelling argument"
#define SAY_END_1 "I'll double the worker shifts and put in my own money to get those ships seaworthy and back on the schedule. Count on it!"
class npc_bilgewater_foreman : public CreatureScript
{
public:
    npc_bilgewater_foreman() : CreatureScript("npc_bilgewater_foreman") { }
    CreatureAI* GetAI(Creature* pCreature) const
    {
        return new npc_bilgewater_foremanAI (pCreature);
    }
    bool OnGossipHello(Player* pPlayer, Creature* pCreature)
    {
        if(pPlayer->GetQuestStatus(26337) == QUEST_STATUS_INCOMPLETE)
        {
            pPlayer->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, FOREMAN_GOSSIP_OPTION_1, GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF+1);
        }
    ...
    bool OnGossipSelect(Player* pPlayer, Creature* pCreature, uint32 /*uiSender*/, uint32 uiAction)
    {
        ...
        switch (uiAction)
        {
            case GOSSIP_ACTION_INFO_DEF+1:
            {
                pCreature->MonsterSay(SAY_START_1,LANG_UNIVERSAL,NULL);
                CAST_AI(npc_bilgewater_foreman::npc_bilgewater_foremanAI, pCreature->AI())->Start = true;
                CAST_AI(npc_bilgewater_foreman::npc_bilgewater_foremanAI, pCreature->AI())->plrGUID = pPlayer->GetGUID();
                break;
            }
        }
        return true;
    }    
    struct npc_bilgewater_foremanAI : public ScriptedAI
    {
        npc_bilgewater_foremanAI(Creature *c) : ScriptedAI(c) {}
        uint32 sayTimer;
        bool Start;
        uint64 plrGUID;
        uint32 say_phase;
        ...
        void UpdateAI(const uint32 diff)
        {
            if(!Start)
                return;            
            Player* pPlayer = Unit::GetPlayer(*me, plrGUID);
            if(!pPlayer)
                return;                
            if(!FactionChanged)
            {
                //Change faction here
                me->setFaction(2165);
                me->AddThreat(pPlayer, 32.0f);
                me->AI()->AttackStart(pPlayer);
                FactionChanged = true;
            }
            if(me->HealthBelowPct(18) && !Completed) // Surrender
            {
                pPlayer->CompleteQuest(26337);
                me->MonsterSay(BATTLE_END,LANG_UNIVERSAL,NULL);
                me->AttackStop();
                me->RestoreFaction();
                pPlayer->AttackStop();
                sayTimer = 3000;
            ...
            if(sayTimer <= diff)
            {
                switch(say_phase)
                {
                    case 1:
                    {
                        me->MonsterSay(BATTLE_SAY_1,LANG_UNIVERSAL,NULL);
                        say_phase = 2;
                        sayTimer = 3000;
                        break;
                    }
                    case 2:
                    {
                        me->MonsterSay(BATTLE_SAY_2,LANG_UNIVERSAL,NULL);
                        say_phase = 3;
                        sayTimer = 3000;
                        break;
                    }
                    case 3:
                    {
                        me->MonsterSay(BATTLE_SAY_3,LANG_UNIVERSAL,NULL);
                        say_phase = 4;
                        sayTimer = 1500;
                        break;
                    }
                    case 4:
                    {
                        me->MonsterSay(BATTLE_SAY_4,LANG_UNIVERSAL,NULL);
                        say_phase = 0;
                        sayTimer = 1500;
                        break;
                    }
                    case 5:
                    {
                        me->MonsterSay(SAY_END_1,LANG_UNIVERSAL,NULL);
                        say_phase = 0;
                        sayTimer = 1500;
                        break;
                    }
