...
enum eDealWell
{
    QUEST_AMPHITHEATER_ANGUISH_GINORMUS           = 25067,
    QUEST_AMPHITHEATER_ANGUISH_ZUMONGA            = 25094,
    QUEST_AMPHITHEATER_ANGUISH_DREADSHREDDER_A    = 25513,
    QUEST_AMPHITHEATER_ANGUISH_STEELSPARK_H       = 25591,
    QUEST_AMPHITHEATER_ANGUISH_ZARINEXX           = 25095, 
    NPC_GINORMUS                                  = 39075,
    NPC_ZUMONGA                                   = 39148,
    NPC_SARINEXX                                  = 39149,
    NPC_DREADSHREDDER                             = 41077,
    NPC_STEELSPARK                                = 41083, 
...
{
    {-7137.606934f, -3787.222168f, 9.051784f, 6.091538f}, // They all spawn here
};
...
*/
class dr_dealwell : public CreatureScript
{
public:
    dr_dealwell() : CreatureScript("dr_dealwell") { }
    struct dr_dealwellAI : public ScriptedAI
    {
        dr_dealwellAI(Creature* pCreature) : ScriptedAI(pCreature) {}
        uint64 SummonGUID;
        uint64 uiPlayerGUID;
        uint32 uiTimer;
        uint32 uiPhase;
        ...
        void SetData(uint32 uiId, uint32 uiValue)
        {
            ...
            switch(uiId)
            {
                case 1:
                    switch(uiValue)
                    {
                        case QUEST_AMPHITHEATER_ANGUISH_GINORMUS:
                            //DoScriptText(SAY_QUEST_ACCEPT_TUSKARRMAGEDON, me);
                            uiPhase = 1;
                            uiTimer = 4000;
                            break;
                        case QUEST_AMPHITHEATER_ANGUISH_ZUMONGA:
                            //DoScriptText(SAY_QUEST_ACCEPT_KORRAK_1, me);
                            uiPhase = 3;
                            uiTimer = 3000;
                            break;
                        case QUEST_AMPHITHEATER_ANGUISH_ZARINEXX:
                            uiPhase = 6;
                            uiTimer = 3000;
                            break;
                        case QUEST_AMPHITHEATER_ANGUISH_DREADSHREDDER_A:
                        case QUEST_AMPHITHEATER_ANGUISH_STEELSPARK_H:
                            uiTimer = 5000;
                            uiPhase = 7;
                            break;
                    }
        ...
        void UpdateAI(const uint32 uiDiff)
        {
            ...
            if (uiPhase)
            {
                ...
                if (uiTimer <= uiDiff)
                {
                    ...
                    {
                        case 1:
                            if (Creature* pSummon = me->SummonCreature(NPC_GINORMUS, SpawnPosition[0], TEMPSUMMON_CORPSE_DESPAWN, 1000))
                                SummonGUID = pSummon->GetGUID();
                            uiPhase = 2;
                            uiTimer = 4000;
                            break;
                        case 3:
                            if (Creature* pSummon = me->SummonCreature(NPC_ZUMONGA, SpawnPosition[0], TEMPSUMMON_CORPSE_DESPAWN, 1000))
                                SummonGUID = pSummon->GetGUID();
                            uiPhase = 4;
                            uiTimer = 4000;
                            break;
                        case 6:
                            if (Creature* pSummon = me->SummonCreature(NPC_SARINEXX, SpawnPosition[0], TEMPSUMMON_CORPSE_DESPAWN, 1000))
                                SummonGUID = pSummon->GetGUID();
                            uiTimer = 3000;
                            uiPhase = 5;
                            break;
                        case 7:
                            {
                               if (!pPlayer)
                                   return;
                                uint8 plrRace = pPlayer->getRace();
                                if(plrRace == 2 || plrRace == 5 || plrRace == 6 || plrRace == 8 || plrRace == 9 || plrRace == 10) //Player is horde, ToDo: Find a better way
                                {
                                    if (Creature* pSummon = me->SummonCreature(NPC_STEELSPARK, SpawnPosition[0], TEMPSUMMON_CORPSE_DESPAWN, 1000))
                                        SummonGUID = pSummon->GetGUID();
                                }
                                else
                                    if (Creature* pSummon = me->SummonCreature(NPC_DREADSHREDDER, SpawnPosition[0], TEMPSUMMON_CORPSE_DESPAWN, 1000))
                                        SummonGUID = pSummon->GetGUID();
                                uiPhase=8;
                            }
                            break;
    ...
    CreatureAI *GetAI(Creature *creature) const
    {
        return new dr_dealwellAI(creature);
    }
};
