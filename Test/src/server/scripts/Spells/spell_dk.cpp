...

{
    ...
    SPELL_DK_DISPLAY_GHOUL_CORPSE		= 25537,
};
...

    {
...

        {
...

            {
                ...
                float absorb = GetCaster()->GetTotalAttackPowerValue(BASE_ATTACK) * 0.75f + target->GetHealthGain(absorb);

                target->GetHealthGain(absorb);
...
public:
    ...
    class spell_dk_blood_tap_SpellScript : public SpellScript
    {
        ...
        void Register()
        {
            OnEffectHitTarget += SpellEffectFn(spell_dk_blood_tap_SpellScript::HandleRune, EFFECT_0, SPELL_EFFECT_ACTIVATE_RUNE);
        }       
    };
    SpellScript* GetSpellScript() const
    {
...
public:
    ...
    class spell_dk_runic_power_feed_SpellScript : public SpellScript
    {
        ...
        bool Validate(SpellInfo const * /*spellEntry*/)
        {
            if (!sSpellStore.LookupEntry(SPELL_DK_SUMMON_GARGOYLE))
                return false;
            return true;
        }
        ...
        void HandleDummy(SpellEffIndex /*effIndex*/)
        {
            if (Unit* caster = GetCaster())
            {
                ...
                if (caster->GetPower(POWER_RUNIC_POWER) < 30)
                    caster->RemoveAurasDueToSpell(SPELL_DK_SUMMON_GARGOYLE, caster->GetGUID());
                else
        ...
        void Register()
        {
            OnEffectHitTarget += SpellEffectFn(spell_dk_runic_power_feed_SpellScript::HandleDummy, EFFECT_0, SPELL_EFFECT_DUMMY);
        }
    };
    SpellScript* GetSpellScript() const
    {
