...
enum DruidSpells
{
    SPELL_DRUID_ECLIPSE_GENERAL_ENERGIZE    = 79577,
    ...
    SPELL_DRUID_LUNAR_ECLIPSE_MARKER	    = 67484,
    ...
    SPELL_DRUID_SOLAR_ECLIPSE_MARKER	    = 67483,
        ...
        class spell_dru_ferocious_bite_SpellScript : public SpellScript
        {
            ...
            void CalculateDamage(SpellEffIndex /*effIndex*/)
            {
                if (Unit* caster = GetCaster())
                {
                    ...
                    damage += int32(CalculatePct(caster->ToPlayer()->GetComboPoints() * ap, 7));
                    SetHitDamage(damage);
                }
            }
...
public:
	spell_dru_rejuvenation() :
			SpellScriptLoader("spell_dru_rejuvenation") {
	}
	...
	class spell_dru_rejuvenation_SpellScript: public SpellScript {
		...
		void Register() {
			OnEffectHitTarget += SpellEffectFn(spell_dru_rejuvenation_SpellScript::HandleDummy, EFFECT_0, SPELL_EFFECT_APPLY_AURA);
		}
	};
