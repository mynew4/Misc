...
class achievement_jugger_not : public AchievementCriteriaScript
{
    public:
        achievement_jugger_not() : AchievementCriteriaScript("achievement_jugger_not") { }

     bool OnCheck(Player* source, Unit* /*target*/) {
		if (source->GetBattlegroundTypeId() != BATTLEGROUND_BFG)
			return false;
		...
		if (!pEotS)
            ...
            if (!static_cast<BattlegroundBFG*>(pEotS)->IsTeamScores500Disadvantage(source->GetTeam()))
                return false;
