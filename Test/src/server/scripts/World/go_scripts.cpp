...
public:
	go_rusty_cage() :
			GameObjectScript("go_rusty_cage") {
	}

	bool OnGossipHello(Player *pPlayer, GameObject *pGO) {
		if (Creature *pGoblinPrisoner = pGO->FindNearestCreature(NPC_GOBLIN_PRISIONER, 5.0f, true)) {
			pGO->SetGoState(GO_STATE_ACTIVE);
			pPlayer->KilledMonsterCredit(NPC_GOBLIN_PRISIONER,
					pGoblinPrisoner->GetGUID());
...
public:
	go_black_cage() :
			GameObjectScript("go_black_cage") {
	}

	bool OnGossipHello(Player *pPlayer, GameObject *pGO) {
		if ((pPlayer->GetTeamId() == TEAM_ALLIANCE
				&& pPlayer->GetQuestStatus(
						QUEST_ALLIANCE_YOU_VE_REALLY_DONE_IT_THIS_TIME_KUL)
						== QUEST_STATUS_INCOMPLETE)
				|| (pPlayer->GetTeamId() == TEAM_HORDE
						&& pPlayer->GetQuestStatus(
								QUEST_HORDE_YOU_VE_REALLY_DONE_IT_THIS_TIME_KUL)
								== QUEST_STATUS_INCOMPLETE)) {
			Creature *pPrisoner = pGO->FindNearestCreature(NPC_CAPTIVE_ASPIRANT,
					1.0f);
			if (!pPrisoner)
...
};class go_archaeology_find : public GameObjectScript
{
public:
    go_archaeology_find() : GameObjectScript("go_archaeology_find") {}

    bool OnGossipHello(Player *pPlayer, GameObject *pGO)
    {

        uint8 count = urand(3,6) * PLAYER_CURRENCY_PRECISION;

        if (pPlayer->HasSkill(794))
        {
            switch(pGO->GetEntry())
            {
                case 207190: // Tol'Vir Archaeology Find
                    pPlayer->ModifyCurrency(401, count);
                    break;
                case 207189: // Vrykul Archaeology Find
                    pPlayer->ModifyCurrency(399, count);
                    break;
                case 207188: // Draenei Archaeology Find
                    pPlayer->ModifyCurrency(398, count);
                    break;
                case 207187: // Orc Archaeology Find
                    pPlayer->ModifyCurrency(397, count);
                    break;
                case 206836: // Fossil Archaeology Find
                    pPlayer->ModifyCurrency(393, count);
                    break;
                case 204282: // Dwarf Archaeology Find
                    pPlayer->ModifyCurrency(384, count);
                    break;
                case 203078: // Nerubian Archaeology Find
                    pPlayer->ModifyCurrency(400, count);
                    break;
                case 203071: // Night Elf Archaeology Find
                    pPlayer->ModifyCurrency(394, count);
                    break;
                case 202655: // Troll Archaeology Find
                    pPlayer->ModifyCurrency(385, count);
                    break;
            }
        }
