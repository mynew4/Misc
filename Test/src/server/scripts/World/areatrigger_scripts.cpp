...
enum FrostgripsHollow
{
	...
	AT_DOWN_AT_THE_DOCKS			= 3546,
	AT_GATEWAY_TO_THE_FRONTIER		= 3547,
	AT_LORDAERON_THRONE_ROOM		= 3548,
	AT_BOUGHT_OF_ETERNALS			= 3549,
	AT_SPOOKY_LIGHTHOUSE			= 3551,
	AT_STONEWROUGHT_DAM				= 3552,
	AT_DARK_PORTAL					= 4356,
};
...
public:
	...
	bool OnTrigger(Player* player, AreaTriggerEntry const* trigger) {
		...
		case TRIGGER_SOUTH:
			...
			break;
		case TRIGGER_CENTRAL:
			player->KilledMonsterCredit(NPC_CENTRAL_GATE, 0);
			break;
		case TRIGGER_NORTH:
			player->KilledMonsterCredit(NPC_NORTH_GATE, 0);
			break;
		case TRIGGER_NORTHWEST:
			player->KilledMonsterCredit(NPC_NORTHWEST_GATE, 0);
			break;
		}
		return true;
	}
	...
	bool OnTrigger(Player* player, AreaTriggerEntry const* trigger) {
		if (!player->isDead()
			...
			case AT_SLAUGHTERHOUSE:
				...
				break;
			case AT_GRAINERY:
				player->KilledMonsterCredit(NPC_CREDIT_GRAINERY, 0);
				break;
			case AT_TORP_FARM:
				player->KilledMonsterCredit(NPC_CREDIT_TORP_FARM, 0);
				break;
			}
		}
		return true;
