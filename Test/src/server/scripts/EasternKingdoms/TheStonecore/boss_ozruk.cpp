...
#define SAY_DEATH          "Un protecteur est tombé. Le cœur du monde est vulnérable !"
#define SAY_ELEMENTIUM     "Venez vous briser sur mon corps. Sentez la force de la terre !"
#define SAY_ENRAGE         "Le cycle est terminé."
...
#define SPELL_GROUND_SLAM                 78903
         ...
         void UpdateAI(const uint32 diff)
         {
            ...
            if (FrostFeverTimer <= diff)
            {
             if (Unit *target = SelectTarget(SELECT_TARGET_RANDOM, 0))
                 DoCast(target, RAND(SPELL_FROST_FEVER, SPELL_SLOW));
                 FrostFeverTimer = urand(6000, 9000);
                 }else
