    ...
    struct npc_gilneas_city_guard_phase1AI : public ScriptedAI
    {
        ...
        void UpdateAI(const uint32 diff)
        {
            if (me->GetGUIDLow() == 3486400)
            {
                if (tSay <= diff)
                {
                    Talk(RAND(SAY_GILNEAS_CITY_GUARD_GATE_1, SAY_GILNEAS_CITY_GUARD_GATE_2, SAY_GILNEAS_CITY_GUARD_GATE_3));
                    tSay = DELAY_SAY_GILNEAS_CITY_GUARD_GATE; // Reset timer
                }
                else
                {
        ...
        void UpdateAI(const uint32 diff)
        {
            if (!UpdateVictim())
            {
                ...
                if (doYell)
                {
                    if (tYell <= diff)
                    {
                        Talk(RAND(YELL_PRINCE_LIAM_GREYMANE_1, YELL_PRINCE_LIAM_GREYMANE_2, YELL_PRINCE_LIAM_GREYMANE_3, YELL_PRINCE_LIAM_GREYMANE_4, YELL_PRINCE_LIAM_GREYMANE_5));
                        doYell = false;
                    }
                    else
                        tYell -= diff;
...
public:
    ...
    struct npc_frightened_citizenAI : public ScriptedAI
    {
        ...
        Paths LoadPaths()
        {
            ...
            {
                TC_LOG_ERROR(LOG_FILTER_SERVER_LOADING,"waypoint_data for frightened citizen missing");
                    ...
                    if (tEvent <= diff)
                    {
                        switch (Phase)
                        {
                            ...
                            case (1):
                            {
                                ...
                                player->GetMotionMaster()->MoveKnockbackFrom(-1791.94f, 1427.29f, 12.4584f, 22.0f);
        ...
        void UpdateAI(const uint32 diff)
        {
            if (!me->getVictim() && me->GetPhaseMask() == 1)
            {
                ...
                if (guid_panicked_nextsay == me->GetGUIDLow())
                {
                    ...
                    {
                        Talk(RAND(SAY_PANICKED_CITIZEN_1, SAY_PANICKED_CITIZEN_2, SAY_PANICKED_CITIZEN_3, SAY_PANICKED_CITIZEN_4));
...
class spell_rescue_krennan : public SpellScriptLoader
{
	...
	void HandleDummy(SpellEffIndex /*effIndex*/)
	{
 		...
 		if (Creature *krennan = caster->FindNearestCreature(35907, 30))
 		    caster->GetVehicle()->AddPassenger(krennan, false);
 		if (caster->GetTypeId() != TYPEID_PLAYER)
 		    return;
 	...
 	void Register()
 	{
 	    OnEffectLaunch += SpellEffectFn(spell_rescue_krennan_SpellScript::HandleDummy, EFFECT_0, SPELL_EFFECT_SUMMON);
 	}
    };
    ...
    struct npc_lieutenant_waldenAI : public ScriptedAI
    {
        ...
        void sQuestReward(Player *player, const Quest *pQuest, uint32 data)
        {
            if (pQuest->GetQuestId() == QUEST_LOCKDOWN && player->GetPhaseMask() == 1)
                player->SetAuraStack(SPELL_PHASE_QUEST_2, player, 1); //phaseshift
        }
    };
};
void AddSC_gilneas()
