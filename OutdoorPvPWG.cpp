    ...
    else if (obj->GetGoType() == GAMEOBJECT_TYPE_DESTRUCTIBLE_BUILDING)
    {
        ...
        if (eventId == obj->GetGOInfo()->building.damagedEvent)
        {
            ...
            switch(state->type)
            {
                ...
                case BUILDING_WALL:
                {
                    sWorld->SendZoneText(ZONE_WINTERGRASP, sObjectMgr->GetArkCoreStringForDBCLocale(LANG_BG_WG_FORTRESS_UNDER_ATTACK));
...
bool OutdoorPvPWG::Update(uint32 diff)
{
    ...
    if (m_timer > diff)
    {
        ...
        if (isWarTime())
        {
            ...
            if (m_timer != 1)
                sWorld->SendZoneText(ZONE_WINTERGRASP, sObjectMgr->GetArkCoreStringForDBCLocale(LANG_BG_WG_BATTLE_STARTS));
            StartBattle();
        }
...
{// Uptime will do all the work
    ...
    {
        m_timer = 1;
        sWorld->SendZoneText(ZONE_WINTERGRASP, sObjectMgr->GetArkCoreStringForDBCLocale(LANG_BG_WG_BATTLE_FORCE_START));
    }
    sWorld->SendWintergraspState();
}
...
{// Uptime will do all the work.
    ...
    {
        m_timer = 1;
        sWorld->SendZoneText(ZONE_WINTERGRASP, sObjectMgr->GetArkCoreStringForDBCLocale(LANG_BG_WG_BATTLE_FORCE_STOP));
    }
       sWorld->SendWintergraspState();
}
